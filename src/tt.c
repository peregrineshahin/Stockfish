...
void tt_allocate(size_t mbSize) {
  ...
  if (settings.largePages) {
    ...
    TT.mem =
        VirtualAlloc(NULL, lpSize, MEM_COMMIT | MEM_RESERVE | MEM_LARGE_PAGES,
                     PAGE_READWRITE);
    if (!TT.mem)
  ...
  TT.table =
      (Cluster *)((((uintptr_t)TT.mem) + alignment - 1) & ~(alignment - 1));
  if (!TT.mem)
    goto failed;
...
void tt_clear(void) {
  ...
  if (TT.table) {
    for (int idx = 0; idx < Threads.numThreads; idx++)
      thread_wake_up(Threads.pos[idx], THREAD_STATE_TT_CLEAR);
    for (int idx = 0; idx < Threads.numThreads; idx++)
