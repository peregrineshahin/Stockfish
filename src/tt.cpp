...
struct TTEntry {
    ...
    TTData read() const {
        return TTData{Move(move16), Value(value16), Depth(depth8 + DEPTH_ENTRY_OFFSET),
...
void TTEntry::save(Key k, Value v, bool pv, Bound b, Depth d, Move m, uint8_t generation8) {
...
void TTWriter::write(Key k, Value v, bool pv, Bound b, Depth d, Move m, uint8_t generation8) {
