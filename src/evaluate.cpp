...
Value Eval::evaluate(const Position& pos) {
  ...
  // Damp down the evaluation when shuffling by the difference between root 50mc and pos 50mc
  const int damping = std::max((pos.rule50_count() - pos.this_thread()->root50mc), 0);
  v = v * (200 - damping) / 214;
