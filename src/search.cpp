...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (depth <= 0)
        return qsearch < PvNode ? PV : NonPV > (pos, ss, alpha, beta, 0, cutNode);
    ...
    {
        value = qsearch<NonPV>(pos, ss, alpha - 1, alpha, 0, cutNode);
    ...
    if (depth <= 0)
        return qsearch<PV>(pos, ss, alpha, beta, 0, cutNode);
        ...
        && !(ttData.depth >= depth - 3 && ttData.value != VALUE_NONE && ttData.value < probCutBeta))
    {
        ...
        while ((move = mp.next_move()) != Move::none())
        {
            ...
            value = -qsearch<NonPV>(pos, ss + 1, -probCutBeta, -probCutBeta + 1, 0, !cutNode);
...
Value Search::Worker::qsearch(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    Depth movePickerDepth = !(PvNode || cutNode) ? DEPTH_QS_NORMAL : DEPTH_QS_CHECKS;
    ...
    // At non-PV nodes we check for an early TT cutoff
    if (!PvNode && ttData.depth >= movePickerDepth
        && ttData.value != VALUE_NONE  // Can happen when !ttHit or when access race in probe()
        && (ttData.bound & (ttData.value >= beta ? BOUND_LOWER : BOUND_UPPER)))
        return ttData.value;
    ...
    // all evasions).
    Square     prevSq = ((ss - 1)->currentMove).is_ok() ? ((ss - 1)->currentMove).to_sq() : SQ_NONE;
    MovePicker mp(pos, ttData.move, movePickerDepth, &thisThread->mainHistory,
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        thisThread->nodes.fetch_add(1, std::memory_order_relaxed);
        pos.do_move(move, st, givesCheck);
        value = -qsearch<nodeType>(pos, ss + 1, -beta, -alpha, depth - 1, !(PvNode || cutNode));
        pos.undo_move(move);
