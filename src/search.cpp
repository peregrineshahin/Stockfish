...
moves_loop:  // When in check, search starts here
    ...
    auto bound = bestValue >= beta ? BOUND_LOWER : PvNode && bestMove ? BOUND_EXACT : BOUND_UPPER;

    // Write gathered information in transposition table
    // Static evaluation is saved as it was before correction history
    if (!excludedMove && !(rootNode && thisThread->pvIdx))
        ttWriter.write(posKey, value_to_tt(bestValue, ss->ply), PvNode && (bound & BOUND_UPPER),
                       ss->ttPv, bound, depth, bestMove, unadjustedStaticEval, tt.generation());
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    // Static evaluation is saved as it was before adjustment by correction history
    ttWriter.write(posKey, value_to_tt(bestValue, ss->ply), false, pvHit,
