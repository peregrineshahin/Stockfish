...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (((!PvNode && tte->depth() > depth) || (ttValue < alpha - 433 - (302 - 141 * ((ss + 1)->cutoffCnt > 3)) * depth * depth))
        && !excludedMove && ttValue != VALUE_NONE  // Possible in case of TT access race or if !ttHit
