...
moves_loop: // When in check, search starts here
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    if (!pos.capture_stage(bestMove))
    {
        ...
        {
            int penality = quietsSearched[i] != ss->killers[1] ? -bestMoveBonus : -bestMoveBonus/2;
            thisThread->mainHistory[us][from_to(quietsSearched[i])] << penality;
            update_continuation_histories(ss, pos.moved_piece(quietsSearched[i]), to_sq(quietsSearched[i]), penality);
        }
    }
    else
    {
