...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    else if (ss->ttHit)
    {
        ...
        if (eval == VALUE_NONE)
            ss->staticEval = eval = evaluate(pos, is_ok((ss-2)->currentMove) ? (ss-2)->staticEval : 0);
        else if (PvNode)
            Eval::NNUE::hint_common_parent_position(pos);
    ...
    else
    {
        if (ss->ttHit)
        {
            ...
            if ((ss->staticEval = bestValue = tte->eval()) == VALUE_NONE)
                ss->staticEval = bestValue = evaluate(pos, is_ok((ss-2)->currentMove) ? (ss-2)->staticEval : 0);
            ...
            ss->staticEval = bestValue = (ss-1)->currentMove != MOVE_NULL ? evaluate(pos, is_ok((ss-2)->currentMove) ? (ss-2)->staticEval : 0)
                                                                          : -(ss-1)->staticEval;
