...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    assert(pos.upcoming_repetition(ss->ply, (ss - 1)->currentMove, (ss - 2)->currentMove,
                                   (ss - 3)->currentMove)
           == pos.verified_upcoming_repetition(ss->ply));
    // Check if we have an upcoming move that draws by repetition.
    if (!rootNode && alpha < VALUE_DRAW
        && pos.upcoming_repetition(ss->ply, (ss - 1)->currentMove, (ss - 2)->currentMove,
                                   (ss - 3)->currentMove))
    {
