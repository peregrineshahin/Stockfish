...
namespace Stockfish {
    ...
    namespace {
        ...
        // Skill structure is used to implement strength limit. If we have an uci_elo
        // then we convert it to a suitable fractional skill level using anchoring to
        // CCRL Elo (goldfish 1.13 = 2000) and a fit through Ordo derived Elo for a
        // match (TC 60+0.6) results spanning a wide range of k values.
        struct Skill {
        ...
        void  update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue,
        ...
        template<bool Root> uint64_t perft(Position& pos, Depth depth) {
            ...
            for (const auto& m : MoveList<LEGAL>(pos)) {
                ...
                if (Root) sync_cout << UCI::move(m, pos.is_chess960()) << ": " << cnt << sync_endl;
            }
            return nodes;
        }
    ...
    void MainThread::search() {
        ...
        if (Limits.npmsec) Time.availableNodes += Limits.inc[us] - Threads.nodes_searched();
    ...
    void Thread::search() {
        ...
        while (++rootDepth < MAX_PLY && !Threads.stop &&
            ...
            // If the skill level is enabled and time is up, pick a sub-optimal best
            // move
            if (skill.enabled() && skill.time_to_pick(rootDepth)) skill.pick_best(multiPV);
        ...
        // If the skill level is enabled, swap the best PV line with the sub-optimal
        // one
        if (skill.enabled())
    ...
    namespace {
        ...
        Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
            ...
            if (thisThread == Threads.main()) static_cast<MainThread*>(thisThread)->check_time();

            // Used to send selDepth info to GUI (selDepth counts from 1, ply from 0)
            if (PvNode && thisThread->selDepth < ss->ply + 1) thisThread->selDepth = ss->ply + 1;
            ...
            if (!rootNode) {
                ...
                // with reversed signs apply also in the opposite condition of being mated
            ...
            // to save indentation, we list the condition in all code between here and
            // there.
            ...
            if (!PvNode && !excludedMove && tte->depth() > depth &&
                ...
                if (ttMove) {
                    if (ttValue >= beta) {
                        ...
                        // Extra penalty for early quiet moves of the previous ply (~0 Elo on
                        // STC, ~2 Elo on LTC)
            ...
            if (ss->inCheck) {
                ...
                // Providing the hint that this node's accumulator will be used often brings
                // significant Elo gain (13 Elo)
            ...
            if (is_ok((ss - 1)->currentMove) && !(ss - 1)->inCheck && !priorCapture) {
                int bonus =
                  std::clamp(-18 * int((ss - 1)->staticEval + ss->staticEval), -1817, 1817);
            ...
            if (!PvNode && (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < 17329 &&
                ...
                if (nullValue >= beta) {
                    ...
                    if (thisThread->nmpMinPly || depth < 14) return nullValue;
            ...
            // depth is greater than or equal to the current depth). Use qsearch if depth
            // is equal or below zero (~9 Elo)
            if (PvNode && !ttMove) depth -= 2 + 2 * (ss->ttHit && tte->depth() >= depth);
            ...
            // returns a value much above beta, we can (almost) safely prune the previous
            // move.
            if (!PvNode && depth > 3 &&
...
moves_loop:  // When in check, search starts here
            ...
            while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE) {
                ...
                // Step 14. Pruning at shallow depth (~120 Elo). Depth conditions are
                ...
                if (!rootNode && pos.non_pawn_material(us) &&
                    ...
                    if (capture || givesCheck) {
                        ...
                        if (!pos.see_ge(move, Value(-205) * depth)) continue;
                    } else {
                        ...
                        if (lmrDepth < 6 && history < -3832 * depth) continue;
                        ...
                        if (!pos.see_ge(move, Value(-31 * lmrDepth * lmrDepth))) continue;
                    }
                }
                ...
                if (ss->ply < thisThread->rootDepth * 2) {
                    ...
                    // ttMove. Note that depth margin and singularBeta margin are known for
                    // having non-linear scaling. Their values are optimized to time controls
                    // of 180+1.8 and longer so changing them requires tests at this type of
                    // time controls.
                    if (!rootNode &&
                        depth >=
                          4 - (thisThread->completedDepth > 22) + 2 * (PvNode && tte->is_pv()) &&
                        move == ttMove &&
                        ...
                        tte->depth() >= depth - 3) {
                        Value singularBeta =
                          ttValue - (82 + 65 * (ss->ttPv && !PvNode)) * depth / 64;
                        ...
                        if (value < singularBeta) {
                            ...
                            if (!PvNode && value < singularBeta - 21 &&
                        ...
                        // prune the whole subtree by returning a softbound.
                        ...
                        // If we are on a cutNode, reduce it based on depth (negative extension)
                        // (~1 Elo)
                        else if (cutNode)
                            extension = depth < 17 ? -3 : -1;

                        // If the eval of ttMove is less than value, we reduce it (negative
                        // extension) (~1 Elo)
                ...
                // Decrease reduction if position is or has been on the PV and not likely to
                // fail low. (~3 Elo) Decrease further on cutNodes. (~1 Elo)
                if (ss->ttPv && !likelyFailLow) r -= cutNode && tte->depth() >= depth ? 3 : 2;
                ...
                if (move == (ss - 4)->currentMove && pos.has_repeated()) r += 2;
                ...
                if (depth >= 2 && moveCount > 1 + (PvNode && ss->ply <= 1) &&
                    ...
                    if (value > alpha && d < newDepth) {
                        ...
                        const bool doEvenDeeperSearch =
                          value > alpha + 711 && ss->doubleExtensions <= 6;
                ...
                // Step 18. Full-depth search when LMR is skipped. If expected reduction is
                // high, reduce its depth by 1.
                ...
                // high, otherwise let the parent node fail low with value <= alpha and try
                // another move.
  ...
  */
            ...
            // If there is a move that produces search value greater than alpha we update
            // the stats of searched moves
            else if (bestMove)
            ...
            // Bonus for prior countermove that caused the fail low
                ...
                int bonus = (depth > 5) + (PvNode || cutNode) + (bestValue < alpha - 800) +
                            ((ss - 1)->moveCount > 12);
                update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
        ...
        Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
            ...
            ttDepth =
              ss->inCheck || depth >= DEPTH_QS_CHECKS ? DEPTH_QS_CHECKS : DEPTH_QS_NO_CHECKS;
            ...
            while ((move = mp.next_move()) != MOVE_NONE) {
                ...
                if (bestValue > VALUE_TB_LOSS_IN_MAX_PLY) {
                    ...
                    if (!givesCheck && to_sq(move) != prevSq && futilityBase > -VALUE_KNOWN_WIN &&
                        ...
                        // If static eval is much lower than alpha and move is not winning
                        // material we can prune this move
                        if (futilityBase <= alpha && !pos.see_ge(move, VALUE_ZERO + 1)) {
                            bestValue = std::max(bestValue, futilityBase);
                            continue;
                        }

                        // If static exchange evaluation is much worse than what is needed to
                        // not fall below alpha we can prune this move
                    ...
                    // We prune after the second quiet check evasion move, where being 'in
                    // check' is implicitly checked through the counter, and being a 'quiet
                    // move' apart from being a tt move is assumed after an increment because
                    // captures are pushed ahead.
                    ...
                    if (!pos.see_ge(move, Value(-95))) continue;
                }
                ...
                ss->currentMove = move;
                ss->continuationHistory =
                  &thisThread->continuationHistory[ss->inCheck][capture][pos.moved_piece(move)]
                                                  [to_sq(move)];
